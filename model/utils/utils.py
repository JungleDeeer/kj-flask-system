...
def show_spectrogram(signal, signal2, recnstrtSignal, sr, NFFT, hop_length):
    ...
    librosa.display.specshow(
        librosa.power_to_db(librosa.feature.melspectrogram(y=signal, sr=sr, n_fft=NFFT, hop_length=hop_length),
    ...
    plt.subplot(3, 1, 2)
    librosa.display.specshow(
        librosa.power_to_db(librosa.feature.melspectrogram(y=signal2, sr=sr, n_fft=NFFT, hop_length=hop_length),
                            ref=np.max), sr=sr, x_axis='time', y_axis='linear')
    # librosa.display.specshow(librosa.amplitude_to_db(origianlSpectrogram, ref_power=np.max),sr=sr, x_axis='time',y_axis='linear')
    ...
    plt.subplot(3, 1, 3)
    librosa.display.specshow(
        librosa.power_to_db(librosa.feature.melspectrogram(y=recnstrtSignal, sr=sr, n_fft=NFFT, hop_length=hop_length),
                            ref=np.max), sr=sr, x_axis='time', y_axis='linear')
